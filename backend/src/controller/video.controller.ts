...
export class VideoController {
    ...
    async createNewVideo(@Res() response, @Req() request, @Body() video: Video, @UploadedFiles() files: { video?: Express.Multer.File[], cover?: Express.Multer.File[] }) {
    ...
    @Get('/:id')
    async stream(@Res() response, @Param('id') id, @Req() request) {
        return this.videoService.streamVideo(id, response, request);
    }

    @Put('/:id')
    async update(@Res() response, @Param('id') id, @Body() video: Video) {
        const updatedVid = await this.videoService.update(id, video)
        return response.status(HttpStatus.OK).json(updatedVid)
    }
